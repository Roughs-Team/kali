...
async def ban_message_handler(message: Message):
    if not message.reply_message:
        await message.answer(
            "Перешлите сообщение пользователя, которого хотите забанить или \
            отметьте его в сообщении."
        )
...
async def ban_command_handler(message: Message):
    ...
    await bot.api.messages.remove_chat_user(chat_id=message.peer_id - 2000000000, member_id=user_id)
    await message.answer("Пользователь забанен")
